...
if grade == "Toilet":
    ...
    pm17_toilet = pd.read_excel(
        os.path.join(data_dir, "Final_PM17-Toilet.xlsx")
    )
    feature_config_toilet = {
        'Mean_Yankee Pressure': {
            'min': float(pm17_toilet['Mean_Yankee Pressure'].min()),
            'max': float(pm17_toilet['Mean_Yankee Pressure'].max()),
            'default': 0,
            'step': 1.0,
            'format': "%.2f"
        },
        '% NBKP': {
            'min': float(pm17_toilet['% NBKP'].min()),
            'max': float(pm17_toilet['% NBKP'].max()),
            'default': 0,
            'step': 1.0,
            'format': "%.3f"
        },
        'Mean_Load KWH Refiner': {
            'min': float(pm17_toilet['Mean_Load KWH Refiner'].min()),
            'max': float(pm17_toilet['Mean_Load KWH Refiner'].max()),
            'default': 0,
            'step': 1,
            'format': "%.3f"
        },
        'Mean_Creping': {
            'min': float(pm17_toilet['Mean_Creping'].min()),
            'max': float(pm17_toilet['Mean_Creping'].max()),
            'default': 0,
            'step': 0.1,
            'format': "%.2f"
        },
        'Mean_Jet Wire Ratio': {
            'min': float(pm17_toilet['Mean_Jet Wire Ratio'].min()),
            'max': float(pm17_toilet['Mean_Jet Wire Ratio'].max()),
            'default': 0,
            'step': 0.01,
            'format': "%.3f"
        },
        'GSM': {
            'min': float(pm17_toilet['GSM'].min()),
            'max': float(pm17_toilet['GSM'].max()),
            'default': 0,
            'step': 0.1,
            'format': "%.1f"
        },
        'Mean_Flow Coating': {
            'min': float(pm17_toilet['Mean_Flow Coating'].min()),
            'max': float(pm17_toilet['Mean_Flow Coating'].max()),
            'default': 0,
            'step': 0.1,
            'format': "%.2f"
        },
        'Mean_Flow Release': {
            'min': float(pm17_toilet['Mean_Flow Release'].min()),
            'max': float(pm17_toilet['Mean_Flow Release'].max()),
            'default': 0,
            'step': 0.1,
            'format': "%.2f"
        },
    ...
    st.caption("Percentage of Error (Average) = 4.1%")
    pm17_facial = pd.read_excel(
        os.path.join(data_dir, "Final_PM17-Facial.xlsx")
    )
    feature_config_facial = {
        'Mean_Yankee Pressure': {
            'min': float(pm17_facial['Mean_Yankee Pressure'].min()),
            'max': float(pm17_facial['Mean_Yankee Pressure'].max()),
            'default': 0,
            'step': 1.0,
            'format': "%.2f"
        },
        '% NBKP': {
            'min': float(pm17_facial['% NBKP'].min()),
            'max': float(pm17_facial['% NBKP'].max()),
            'default': 0,
            'step': 1.0,
            'format': "%.3f"
        },
        'Mean_Load KWH Refiner': {
            'min': float(pm17_facial['Mean_Load KWH Refiner'].min()),
            'max': float(pm17_facial['Mean_Load KWH Refiner'].max()),
            'default': 0,
            'step': 1,
            'format': "%.3f"
        },
        'Mean_Creping': {
            'min': float(pm17_facial['Mean_Creping'].min()),
            'max': float(pm17_facial['Mean_Creping'].max()),
            'default': 0,
            'step': 0.1,
            'format': "%.2f"
        },
        'Mean_Jet Wire Ratio': {
            'min': float(pm17_facial['Mean_Jet Wire Ratio'].min()),
            'max': float(pm17_facial['Mean_Jet Wire Ratio'].max()),
            'default': 0,
            'step': 0.01,
            'format': "%.3f"
        },
        'GSM': {
            'min': float(pm17_facial['GSM'].min()),
            'max': float(pm17_facial['GSM'].max()),
            'default': 0,
            'step': 0.1,
            'format': "%.1f"
        },
        'Mean_Flow Coating': {
            'min': float(pm17_facial['Mean_Flow Coating'].min()),
            'max': float(pm17_facial['Mean_Flow Coating'].max()),
            'default': 0,
            'step': 0.1,
            'format': "%.2f"
        },
        'Mean_Flow Release': {
            'min': float(pm17_facial['Mean_Flow Release'].min()),
            'max': float(pm17_facial['Mean_Flow Release'].max()),
            'default': 0,
            'step': 0.1,
            'format': "%.2f"
        },
    ...
    st.caption("Percentage of Error (Average) = 4.1%")
    pm17_towel = pd.read_excel(
        os.path.join(data_dir, "Final_PM17-Towel.xlsx")
    )
    feature_config_towel = {
        'Mean_Yankee Pressure': {
            'min': float(pm17_towel['Mean_Yankee Pressure'].min()),
            'max': float(pm17_towel['Mean_Yankee Pressure'].max()),
            'default': 0,
            'step': 1.0,
            'format': "%.2f"
        },
        '% NBKP': {
            'min': float(pm17_towel['% NBKP'].min()),
            'max': float(pm17_towel['% NBKP'].max()),
            'default': 0,
            'step': 1.0,
            'format': "%.3f"
        },
        'Mean_Load KWH Refiner': {
            'min': float(pm17_towel['Mean_Load KWH Refiner'].min()),
            'max': float(pm17_towel['Mean_Load KWH Refiner'].max()),
            'default': 0,
            'step': 1,
            'format': "%.3f"
        },
        'Mean_Creping': {
            'min': float(pm17_towel['Mean_Creping'].min()),
            'max': float(pm17_towel['Mean_Creping'].max()),
            'default': 0,
            'step': 0.1,
            'format': "%.2f"
        },
        'Mean_Jet Wire Ratio': {
            'min': float(pm17_towel['Mean_Jet Wire Ratio'].min()),
            'max': float(pm17_towel['Mean_Jet Wire Ratio'].max()),
            'default': 0,
            'step': 0.01,
            'format': "%.3f"
        },
        'GSM': {
            'min': float(pm17_towel['GSM'].min()),
            'max': float(pm17_towel['GSM'].max()),
            'default': 0,
            'step': 0.1,
            'format': "%.1f"
        },
        'Mean_Flow Coating': {
            'min': float(pm17_towel['Mean_Flow Coating'].min()),
            'max': float(pm17_towel['Mean_Flow Coating'].max()),
            'default': 0,
            'step': 0.1,
            'format': "%.2f"
        },
        'Mean_Flow Release': {
            'min': float(pm17_towel['Mean_Flow Release'].min()),
            'max': float(pm17_towel['Mean_Flow Release'].max()),
            'default': 0,
            'step': 0.1,
            'format': "%.2f"
        },
...
if predict_button:
    ...
    st.success(f"Hasil Prediksi MDT PM17 ({grade}): {prediction_ratio:.2f}")