...
if grade == "Toilet":
    ...
    pm16_toilet = pd.read_excel(
        os.path.join(data_dir, "Final_PM16-Toilet.xlsx")
    )
    feature_config_toilet = {
        'Mean_Yankee Pressure': {
            'min': float(pm16_toilet['Mean_Yankee Pressure'].min()),
            'max': float(pm16_toilet['Mean_Yankee Pressure'].max()),
            'default': 0,
            'step': 1.0,
            'format': "%.2f"
        },
        ...
        'Mean_Load KWH Refiner': {
            'min': float(pm16_toilet['Mean_Load KWH Refiner'].min()),
            'max': float(pm16_toilet['Mean_Load KWH Refiner'].max()),
            'default': 0,
            'step': 1,
            'format': "%.3f"
        },
    ...
    st.caption("Percentage of Error (Average) = 4.1%")
    pm16_facial = pd.read_excel(
        os.path.join(data_dir, "Final_PM16-Facial.xlsx")
    )
    feature_config_facial = {
        'Mean_Yankee Pressure': {
            'min': float(pm16_facial['Mean_Yankee Pressure'].min()),
            'max': float(pm16_facial['Mean_Yankee Pressure'].max()),
            'default': 0,
            'step': 1.0,
            'format': "%.2f"
        },
        ...
        'Mean_Load KWH Refiner': {
            'min': float(pm16_facial['Mean_Load KWH Refiner'].min()),
            'max': float(pm16_facial['Mean_Load KWH Refiner'].max()),
            'default': 0,
            'step': 1,
            'format': "%.3f"
        },
    ...
    st.caption("Percentage of Error (Average) = 4.1%")
    pm16_towel = pd.read_excel(
        os.path.join(data_dir, "Final_PM16-Towel.xlsx")
    )
    feature_config_towel = {
        'Mean_Yankee Pressure': {
            'min': float(pm16_towel['Mean_Yankee Pressure'].min()),
            'max': float(pm16_towel['Mean_Yankee Pressure'].max()),
            'default': 0,
            'step': 1.0,
            'format': "%.2f"
        },
        ...
        'Mean_Load KWH Refiner': {
            'min': float(pm16_towel['Mean_Load KWH Refiner'].min()),
            'max': float(pm16_towel['Mean_Load KWH Refiner'].max()),
            'default': 0,
            'step': 1,
            'format': "%.3f"
        },
...
if predict_button:
    ...
    st.success(f"Hasil Prediksi MDT PM16 ({grade}): {prediction_ratio:.2f}")